...
(function() {
  ...
  window.Game = {
    init: function() {
      ...
      this.setCursorState(boolValueOrDefault(Game.storageKeys.showCursor, true));
    ...
    hasCompletedMainLevels: function() {
      var j, level, prog;
    ...
    hasCompletedMainLevelsFullStars: function() {
      var j, level, prog;
      for (level = j = 1; j <= 5; level = ++j) {
        prog = localStorage.getItem(Game.storageKeys.levelProgress + ":" + level);
        if (prog === null || prog < 3) {
    ...
    hasCompletedAllLevels: function() {
      var j, level, prog, ref;
      for (level = j = 1, ref = Game.levels_array.length - 1; (1 <= ref ? j <= ref : j >= ref); level = 1 <= ref ? ++j : --j) {
        prog = localStorage.getItem(Game.storageKeys.levelProgress + ":" + level);
        if (prog === null) {
          return false;
        }
    ...
    hasCompletedAllLevelsFullStars: function() {
      var j, level, prog, ref;
      for (level = j = 1, ref = Game.levels_array.length - 1; (1 <= ref ? j <= ref : j >= ref); level = 1 <= ref ? ++j : --j) {
        prog = localStorage.getItem(Game.storageKeys.levelProgress + ":" + level);
        if (prog === null || prog < 3) {
    ...
    stageLevel: function(number = 1) {
      ...
      if (number <= 5 && number >= Game.availableLevel) {
  ...
  Q.scene("levelSummary", function(stage) {
    var buttonBack, buttonNext, columnInP, columnWidth, columnsNo, currentLevel, empty, gutterX, gutterXinP, index, j, lineHeight, marginX, marginXinP, marginY, nextLevelAvailable, performance, previousStars, score, scoreImg, stars, starsContainer, summaryContainer, x;
    // some math
    marginY = Q.height * 0.25;
    marginXinP = 20; // %
    gutterXinP = 8; // %
    ...
    nextLevelAvailable = (currentLevel < Game.levels_array.length - 1) && (currentLevel !== 5);
  ...
  Q.scene("restartOptions", function(stage) {
    var buttonBack, buttonHeight, buttonNext;
    // # some math
    // marginY = Q.height * 0.25
    buttonHeight = 80;
    // marginXinP = 20 # %
  ...
  Q.UI.MenuButton = Q.UI.Button.extend("UI.MenuButton", {
    init: function(p) {
      ...
      return this.on('click', () => {
        ...
        return Game.trackEvent("Restart Button", "clicked");
      });
    }
  });
